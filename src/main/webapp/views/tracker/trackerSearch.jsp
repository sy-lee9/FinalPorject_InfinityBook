			...
			<div class="row" style="margin-bottom: 21%;">
				...
				<div class="search-bar" style="width: 300px; display: inline;">
					<form action="/mypage/trackerSearch.do" role="search" method="get" class="search-box"
						style=" margin-top: -1%; padding-left: 66%;">
						<select name="searchType">
							<option value="Title">제목</option>
							<option value="Author">저자</option>
							<option value="Title">ISBN</option>
						</select>
						<input class="search-field text search-input" placeholder="검색어를 입력하세요." type="search"
							name="searchValue" style="width: 300px;">
							...
							<c:forEach var="book" items="${item.value}" varStatus="status">
								<figure class="product-style" style="margin-bottom: 40px;">
									...
									<img id="cover" src="${book.cover}" class="product-item"
										style="width: 100%; height: 54%; padding: 10;">
									<button onclick="addReadBook('${book.isbn13}','${book.cover}')" class="add-to-cart"
										style="width: 40%; height: 40px; padding: 0; font-size: 15px; background-color: #987559; color: #ffffff; left: 20px;">완독!</button>
									<button onclick="addReadingBook('${book.isbn13}','${book.cover}')" class="add-to-cart"
										style="width: 40%; height: 40px; padding: 0; font-size: 15px; margin-left: 100px; background-color: #987559; color: #ffffff; left: 20px;">
										읽는 중!</button>
									<figcaption>
										<p style="font-weight: 600;">ISBN : ${book.isbn13}</p>
										<h3 id="title" style="font-size: 17; font-weight: 800; height: auto;">
											${book.title}</h3>
